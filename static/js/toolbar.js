...
export function updateStats(data) {
  ...
  const replayRows = Array.isArray(data.replay_events)
    ? data.replay_events.map(event => {
      const mark = state.reviewMarks && state.reviewMarks[event.event_id];
      const markLabel = mark ? escapeHTML(mark.label) : '—';
      const reason = escapeHTML(event.reason);
      return `
      <div class="stat-row"><span class="stat-label">${event.event_type.toUpperCase()}</span><span class="stat-value">${Number(event.price).toFixed(4)}</span></div>
      <div class="stat-row"><span class="stat-label">Reason</span><span class="stat-value">${reason}</span></div>
      <div class="stat-row"><span class="stat-label">Review</span><span class="stat-value">${markLabel}</span></div>
    `;
    }).join('')
    : '';
  ...
  panel.innerHTML = `
    ...
    ${replayRows ? `<div class="stat-section"><div class="stat-title">Replay Events</div>${replayRows}</div>` : ''}
  `;
